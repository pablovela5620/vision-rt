...
fn main() -> Res<()> {
    ...
    // Models FIRST, source LAST: opening the file starts real-time paced playback
    // into a 5-frame buffer, so minutes of first-run work (ONNX download + engine
    // build) would let a short clip play out before the loop consumes a frame —
    // truncating the run while still reporting clean EOS.
